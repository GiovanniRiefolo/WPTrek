...
// Vulcan Blocks - powered by ACF

// --- custom categories
function vc_block_categories( $categories, $post ) {
    return array_merge(
        $categories,
        array(
            array(
                'slug' => 'vulcan-blocks',
                'title' => __( 'Vulcan Blocks', 'vulcan-blocks' ),
            ),
        )
    );
}
add_filter( 'block_categories', 'vc_block_categories', 2, 2 );
// --- registering with ACF blocks
