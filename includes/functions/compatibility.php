<?php
/**
  * WPTrek compatibility
  * Prevents WPTrek from running on WordPress versions prior to 5.5,
...
// Prevent switching to WPTrek on old version of WordPress
...
function wptrek_upgrade_notice() {
	$message = sprintf( __( 'WPTrek requires at least WordPress version 5.5. You are running version %s. Please upgrade and try again.', 'wptrek' ), $GLOBALS['wp_version'] );
...
function wptrek_customize() {
	wp_die(
		sprintf(
			__( 'WPTrek requires at least WordPress version 5.5. You are running version %s. Please upgrade and try again.', 'wptrek' ),
			$GLOBALS['wp_version']
		),
		'',
		array(
...
function wptrek_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( sprintf( __( 'WPTrek requires at least WordPress version 5.5. You are running version %s. Please upgrade and try again.', 'wptrek' ), $GLOBALS['wp_version'] ) );
	}
}
